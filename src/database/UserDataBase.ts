...
export class UserDataBase extends BaseDataBase {
  ...
  public async postUser(user: User) {
    ...
    const result = await BaseDataBase.connection(USER_LIST).insert(UserDB);
    return result;
  }
  public async postProductsUser(productsUser: ProductsUser) {
    const productsUserDB = this.toProductsUserDBModel(productsUser);
    const result = await BaseDataBase.connection(PRODUCTS_USER_LIST).insert(
      productsUserDB
    );
    return result;
  }
